...
function App() {
  ...
  const [text, setText] = useState(
    'The party shall indemnify the other party. Payment of $5000 must be made within 30 days. Either party may terminate on material breach.'
  )
  ...
  async function analyzeDocument() {
    ...
    try {
      ...
      for (let i = 0; i < 40; i += 1) {
        ...
        const payload = (await getRes.json()) as DocumentPayload
        const latest = payload.analyses?.[payload.analyses.length - 1] ?? null
        if (payload.document?.status === 'analyzed' || latest) {
          setAnalysis(latest)
          setDocumentText(payload.textPreview ?? '')
          setStatus('done')
          return
        }
      }
      ...
      if (String(message).includes('Create failed: 400')) {
        setError('Please upload a file or paste non-empty text before analyzing.')
      } else if (String(message).includes('Create failed: 422')) {
        setError('This file appears to have no extractable text. Upload a text-based PDF, DOCX, or TXT.')
      } else {
        setError(message)
      }
    }
    ...
    <div style={shell}>
      <div style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gap: 24 }}>
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
          ...
          <button style={{ ...button, borderRadius: 999 }} onClick={analyzeDocument}>
            Start Analysis
          </button>
        </header>
            ...
            <div style={{ border: '1px dashed rgba(255,255,255,0.18)', borderRadius: 16, padding: 20, minHeight: 240 }}>
              <div style={{ display: 'grid', gap: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  <label htmlFor="file" style={{ opacity: 0.8, fontSize: 12 }}>
                    {file ? file.name : 'Drop or choose a PDF/DOCX/TXT'}
                  </label>
                </div>
          ...
          <div style={{ display: 'grid', gap: 24 }}>
            ...
            <div style={card}>
              ...
              <div style={{ display: 'grid', gap: 10 }}>
                <input style={field} value={chatInput} onChange={(e) => setChatInput(e.target.value)} placeholder="Ask a question about the analysis" />
                <button style={button} onClick={askChat} disabled={!documentId}>
                  Ask
                </button>
        ...
        <section style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 24 }}>
          <div style={card}>
            ...
            <div style={{ opacity: 0.8, lineHeight: 1.55, whiteSpace: 'pre-wrap' }}>{documentText || text || 'No text uploaded yet.'}</div>
          </div>
              ...
              {(analysis?.flags ?? []).slice(0, 5).map((flag) => (
                <div key={flag.clauseId} style={{ border: '1px solid rgba(255,255,255,0.15)', borderRadius: 12, padding: 10 }}>
                  ...
                  <div style={{ opacity: 0.7, fontSize: 12 }}>
                    Confidence: {flag.confidence.toFixed(2)} | Score: {flag.scoreContribution.toFixed(2)}
                  </div>
                </div>
              ))}
